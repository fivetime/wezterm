...
/// 28, the strip's left margin being 28), its icon 16 (Chrome's is the
/// kExpandMoreOldIcon chevron, a 8.6 x 4.3 V at (3.7, 6.44) of the 16).
pub const TAB_SEARCH_BUTTON: f32 = 28.;
pub const TAB_SEARCH_INSET: f32 = 6.;
pub const TAB_SEARCH_ICON: f32 = 16.;
...
pub struct Tab {
    ...
    /// tab too narrow for anything else (tab.cc center_icon_), which
    /// `favicon_centred` says.
    pub favicon: Option<Rect>,
    pub favicon_centred: bool,
...
mod tests {
    ...
    fn the_tab_search_button_before_the_tabs() {
        ...
        // the 16 icon centred in the button
        assert_eq!(l.tab_search_icon, Some(Rect::new(12., 12., 16., 16.)));
